...
class HMC(object):
    ...
    def debug_cleanup(self):
        ''' If debug is off and debug folder exists then clean up the debug folder '''
        if self.debug:
            print("-> Clean up of debug folder skipped")
        if not self.debug:
            print("-> Cleaning up debug folder")
            if platform.system() == 'Linux':
                try:
